  ...
  describe '配送先情報の保存' do
    ...
    context '配送先情報の保存ができないとき' do
      ...
      it '郵便番号が空だと保存できないこと' do
        ...
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include("郵便番号を入力してください", "郵便番号が無効です。ハイフン（-）を含めた形式で入力してください。")
      end
      it '郵便番号にハイフンがないと保存できないこと' do
        @order_form.postcode = 1_234_567
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include("郵便番号が無効です。ハイフン（-）を含めた形式で入力してください。")
      end
      ...
      it '電話番号が空だと保存できないこと' do
        ...
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include("電話番号を入力してください", "電話番号を入力してください（正しい形式で入力してください）")
      end
      it '電話番号にハイフンがあると保存できないこと' do
        @order_form.phone_number = '123 - 1234 - 1234'
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include("電話番号を入力してください（正しい形式で入力してください）")
      end
      ...
      it 'トークンが空だと保存できないこと' do
        ...
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include("クレジットカード情報入力を入力してください")
      end
    end
  end
end
